...
class LabelStudioTools:
    def __init__(self, dataset_location):
        ...
        if not os.path.exists(dataset_location + "/data"):
            os.makedirs(dataset_location + "/data")
        self.data_location = dataset_location + "/data"

        if not os.path.exists(dataset_location + "/predictions"):
            os.makedirs(dataset_location + "/predictions")
    ...
    def save_predictions_as_json(self, s3_data_folder, location, timestamp, session_id, predictions,detections = None, vcg_trigger_detections = None):
        ...
        with open(self.prediction_location + f"/{location}_fgre_{timestamp}_{session_id}.json", "w") as f:
            json.dump(pred, f)
            ...
            if "ecg2_filter" in task:
                ...
                if("choices" in task["ecg2_filter"]):
                    ...
                    for filter in task["ecg2_filter"]["choices"]:
                        if(filter == "MWI Filter"):
                            is_mwi_filter = True
                        if(filter == "Paced"):
                            is_paced_signal = True
                else:
    ...
    def convert_json_annotations_to_dataframe(self, json_file):
        ...
        for task in data:
            ...
            if("ecg2_quality" in task and task["ecg2_quality"] == "Unusable"):
                continue
    ...
    def isPacedSignal(self, annotations):
        ...
        if("result" in annotations[0]):
            ...
            for result in annotations[0]["result"]:
                if("choices" in result["value"] and result["value"]["choices"][0] == "Paced"):
                    return True
        return False
    ...
    def isMWIFilter(self, annotations):
        ...
        if("result" in annotations[0]):
            # For each result in the annotations
            for result in annotations[0]["result"]:
                if("choices" in result["value"] and result["value"]["choices"][0] == "MWI Filter"):
                    return True
        return False
    ...
    def LabelToSeries(self, labels):
        ...
        for label in labels:
            ...
            if(label["instant"] == False):
                # Get the annotation end time
                annotation_end = label["end"]
            
            if("R-Peak" in label["timeserieslabels"] or "PVC-Peak" in label["timeserieslabels"]):
                marked_peaks.append(annotation_start)
    ...
    def AnnotationsToSeries(self, annotations):
        ...
        for annotation in annotations[0]["result"]:
            ...
            # If annotation_value doesn't contain "start" key, then it is not a valid annotation
            if("start" not in annotation_value):
                continue

            # Get the annotation start time
            annotation_start = annotation_value["start"]

            if(annotation_value["instant"] == False):
                # Get the annotation end time
                annotation_end = annotation_value["end"]
            
            if("R-Peak" in annotation_value["timeserieslabels"] or "PVC-Peak" in annotation_value["timeserieslabels"]):
                marked_peaks.append(annotation_start)
        return pd.Series(marked_peaks, copy=True)
