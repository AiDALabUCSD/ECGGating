    ...
    def process_trigger_files(self):
        ...
        for file in self.json_files:
            ...
            if not os.path.exists(file_path):
                ...
                print(f'GE_{file_name}_trigger_data.npy already exists')
    ...
    def process_ecg2_raw_files(self):
        ...
        for file in self.json_files:
            ...
            if not os.path.exists(file_path):
                ...
                print(f'GE_{file_name}_ECG2_data.npy already exists')
...
class UCSDwaveforms:
    ...
    def process_ecg_raw_files(self, ECG = 'ECG2'):
        """
        Read the entire contents of a file, split the file contents by lines,
        convert each line into an integer, and returns the list of integers.
        """
        ...
        for index, row in df.iterrows():
            ...
            if ECG == 'ECG3':
                file_name = f"data_{location}_{timestamp}_{session_id}_ecg3.npy"
            else:
                file_name = f"data_{location}_{timestamp}_{session_id}.npy"
                
            if file_name not in os.listdir("./data/old_preprocessed/") and file_name not in os.listdir(self.out_directory):
                try:
                    raw_ecg_data = ucsd_ds.get_ecg_data(location,timestamp,session_id)
                    if ECG == 'ECG2':
                        np.save(f"{self.out_directory + file_name}",raw_ecg_data[0,:]) # Save the first channel (ECG2)
                    if ECG == 'ECG3':
                        np.save(f"{self.out_directory + file_name}",raw_ecg_data[1,:]) # Save the second channel (ECG3)
