...
class ECGDataset:
    ...
    def __init__(self, dataset_location):
        ...
        subdirs = [x[0] for x in os.walk(dataset_location)][1:] # <-- changed from harcoded "ucsd-ekg-data" to dataset_location

        self.ucsd_ecg_df = pd.DataFrame(columns=["subdir", "file", "location", "lead", "type",  
    ...
    def get_trigger_detections(self, location, timestamp, session_id):
        # Get the file name
        # file_name = self.dataset_location + f"/{location}/ECG2Trig_fgre_{timestamp}_{session_id}"
    ...
    def get_pulse_detections(self, location, timestamp, session_id):
        # Get the file name
        file_name = self.dataset_location + f"/{location}/PPGTrig_fgre_{timestamp}_{session_id}"
    ...
    def get_ecg_data(self, location, timestamp, session_id):
        ...
        # Check if the ecg2File file exists
        if not os.path.exists(ecg2File):
            # Throw exception
            raise IOError(f"File does not exist: {ecg2File}")
        
        # Check if the ecg3File file exists
        if not os.path.exists(ecg3File):
            # Throw exception
            raise IOError(f"File does not exist: {ecg3File}")
    ...
    def get_unique_combinations(self):
        # Only for lead ECG2:
        # From ucsd_ecg_df get number of unique get the unique combinations of session_id, timestamp and location
